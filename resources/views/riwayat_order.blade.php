                        ...
                        @foreach($pesanan->orderItems as $item)
                            <ul class="list-disc list-inside">
                                <li>{{ $item->product->nama  }} × {{ $item->quantity }} =
                                    {{ 'Rp' . number_format($item->harga * $item->quantity, 2, ',', '.') }}</li>
                            </ul>
                        @endforeach
                    </div>
                    {{-- <form action="{{ route('admin.exel') }}" method="GET">
                        <button
                            class="flex items-center gap-2 px-2 py-2 bg-orange3 text-white font-medium rounded-lg shadow hover:bg-orange2 transition duration-300"
